...
def test_federated_cifar100():
    ...
    _, _, train_data_global, test_data_global, data_local_num_dict, train_data_local_dict, test_data_local_dict, _ = load_partition_data_federated_cifar100(
    ...
    for idx in random.sample(range(client_num), test_num):
        ...
        for client_id in get_client_map(client_map_train)[idx]:
            train_local_ds = list(
                iter(tfds.as_numpy(cifar_train.create_tf_dataset_for_client(client_id.decode("utf-8")))))
        ...
        for client_id in get_client_map(client_map_test)[idx]:
            test_local_ds = list(
                iter(tfds.as_numpy(cifar_test.create_tf_dataset_for_client(client_id.decode("utf-8")))))
